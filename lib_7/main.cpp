...
template <typename T>
 ostream& operator<<(ostream &os, const   MyStack2_List<T> &ms) 
{
	 size_t cnt = ms.m_size;
...
----------------------------------------------------------------------------------
	 */
	const MyStack2_List::Node *p = &ms.Head;
	p = p->pNext;
...
int _tmain(int argc, _TCHAR* argv[])
{
	setlocale(LC_ALL, "Russian");
		...
		{
			int *g = q.pop();
			delete g; //Тут как пользователь я делаю с извлечённым данным,что хочу.
		}
		stop
			//Оператор []
